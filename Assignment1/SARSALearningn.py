'''
Program usage:
    python SARSALearningn.py --grid_size 20 --alpha 0.01 --gamma 0.9 --epsilon 0.01 --episodes 10000 --steps 1000 --loggin
    ...
    for episode in range(episodes):
        ...
        for step in range(steps):
            ...
            if next_state[0] < 0 or next_state[0] >= GRID_SIZE or next_state[1] < 0 or next_state[1] >= GRID_SIZE:
                next_state = state
            next_action = grid.epsilon_greedy(next_state, grid.get_actions()[0])
            reward = grid.rewards[(state, action)]
            grid.Q[state][action] = grid.Q[state][action] + alpha * (reward + gamma * grid.Q[next_state][next_action] - grid.Q[state][action])
